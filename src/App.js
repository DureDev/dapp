      ...
      this.state.contract
        ...
        .then(async (data) => {
          ...
          this.state.contract.on(
            "ElectionCreated",
            (id, createdBy, registrationEnd, votingEnd) => {
              this.storeElectionEvents([
                { id, createdBy, registrationEnd, votingEnd },
              ]);
            }
          );
  ...
  async storeElectionEvents(data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let obj of data) {
        ...
        if (
          !tableData.rows.some((item) => String(item[0]) === String(obj.id))
        ) {
          tableData.rows.push([
            String(obj.id),
            String(obj.createdBy),
            String(obj.registrationEnd),
            String(obj.votingEnd),
          ]);
        }
      }

      this.setState({ tableData: tableData, waitingForEvent: false }, () =>
        resolve()
      );
    });
  }
  ...
  async handleCandidateClick(id) {
    ...
    if (!isNaN(id) && this.state.provider) {
      if (Config.isWeb3 && this.state.contract) {
        ...
        contract.registerCandidate(id).catch((err) => console.log(err));
      }
    }
  }
      ...
      <Container className="App">
        <Stack gap={1} className="my-1">
          {(!this.state.userAccount && (
            <Button onClick={this.metamaskLogin.bind(this)}>Login</Button>
          )) || (
            <p className="mx-auto">
              Connected Address: {this.state.userAccount}
            </p>
          )}
          ...
          {this.state.contract && (
            <>
              <CreateElection
                waitingForEvent={this.state.waitingForEvent}
                onClick={this.createElectionHandleClick.bind(this)}
              />
              <GetElection
